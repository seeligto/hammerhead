...
| Sprint 5 | measurement-first sprint; Φ recalibration (A) → **new Φ = +11.3 Elo** (was suspect −10) via HEAD-vs-HEAD 800g at .bestref with Sprint 4B fix overlaid; SB-perf adapter wired + 200g re-baseline (B) → HH 7.0 % Wilson [4.2, 11.4] (+2 pp vs 5 % historical); LMR re-test at 800g/cell (C, both candidates raw −7.8 / corrected −19.1, NO-WINNER); staged-2.5 (D, NPS −5.8 %, corrected arena −82.6, REVERTED); bench-quick 941k stable; one spec commit (`75de950` Φ protocol); `.bestref` UNCHANGED (Outcome B) — headline deliverable is trustworthy Φ + revised cumulative Elo story | ✅ done |
