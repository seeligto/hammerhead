...
def _run_match(label: str, cur_params: dict, opp_params: dict,
               ns, tt_mb: int) -> dict:
    cur_cmd = _in_tree_cmd(tt_mb)
    opp_cmd = _in_tree_with_params_cmd(opp_params, tt_mb)
    ...
    print(f"\n=== {label} (current) vs {opp_params} (opponent) ===",
          flush=True)
    prev_sp = os.environ.get(_SEARCH_PARAMS_ENV)
    prev_ev = os.environ.get(_EVAL_OVERRIDES_ENV)
    os.environ[_SEARCH_PARAMS_ENV] = json.dumps(cur_params)
...
def _parse_config(spec: str) -> tuple[str, dict]:
    """Parse 'label=key:val,key:val' into (label, {key: int(val)})."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--caps", type=int, nargs="+",
                   ...
                   help="flat-cap sweep (each vs --baseline-cap)")
    p.add_argument("--configs", nargs="+", default=None,
                   help="taper configs as 'label=k:v,k:v' (current side); "
                        "overrides --caps when given")
    ...
    opp_params = {"move_gen_cap": ns.baseline_cap}

    if ns.configs:
        jobs = [_parse_config(s) for s in ns.configs]
    else:
        jobs = [(f"cap{c}", {"move_gen_cap": c}) for c in ns.caps]
    print(f"cap H2H: jobs={[j[0] for j in jobs]} baseline={opp_params} "
          f"n_games={ns.n_games} time_ms={ns.time_ms} workers={ns.workers} "
          f"tt_mb={tt_mb} opening_diverse=True", flush=True)

    started = _now_iso()
    rows = [_run_match(label, cur, opp_params, ns, tt_mb)
            for (label, cur) in jobs]

    print("\n" + "=" * 78)
    print(f"{'label':>18}  {'W-L-D':>14}  {'winrate':>8}  "
          f"{'Elo':>7}  {'CI':>16}  verdict")
    for r in rows:
        wld = f"{r['wins']}-{r['losses']}-{r['draws']}"
        ci = f"[{r['elo_ci_lower']:+.0f},{r['elo_ci_upper']:+.0f}]"
        ctrl = " *control*" if r["cur_params"] == opp_params else ""
        print(f"{r['label']:>18}  {wld:>14}  {r['winrate']:>8.4f}  "
              f"{r['elo']:>+7.1f}  {ci:>16}  {r['final_verdict']}{ctrl}")
